...
function convertToText(data) {
    ...
    function parseCapacity(value) {
        ...
        if (!m) return null;
        return { used: Number(m[1]), total: Number(m[2]), raw: raw };
    }

    function num(value) {
        let n = Number(String(value == null ? 0 : value).replace(/[^\d.-]/g, ""));
        return isNaN(n) ? 0 : n;
    }

    let text = "📦 卡通农场小助手仓库统计\n";
    text += "数据仅供参考\n";
    text += `⏱ 运行时长：${hours}小时${minutes}分钟${seconds}秒\n`;

    if (!Array.isArray(data) || data.length === 0) {
        return text + "\n暂无统计数据";
    }

    let totalCoins = 0, totalDiamonds = 0;
    let totalSiloUsed = 0, totalSiloTotal = 0;
    let totalBarnUsed = 0, totalBarnTotal = 0;
    const itemTotals = {};
    const allItems = ["盒钉", "螺钉", "镶板", "螺栓", "木板", "胶带", "土地契约", "木槌", "标桩", "斧头", "木锯", "炸药", "炸药桶", "铁铲", "十字镐", "鱼片", "龙虾尾", "小鸭羽毛", "蜂糖", "蜂蜜", "蜂蜡"];

    data.forEach(item => {
        totalCoins += num(item["金币"]);
        totalDiamonds += num(item["钻石"]);
        let silo = parseCapacity(item["粮仓容量"]);
        let barn = parseCapacity(item["货仓容量"]);
        if (silo) { totalSiloUsed += silo.used; totalSiloTotal += silo.total; }
        if (barn) { totalBarnUsed += barn.used; totalBarnTotal += barn.total; }
        allItems.forEach(name => { itemTotals[name] = (itemTotals[name] || 0) + num(item[name]); });
    });

    text += "\n总览\n";
    text += `💰 ${totalCoins}   |   💎 ${totalDiamonds}   |   🌾 ${totalSiloUsed}//${totalSiloTotal}   |   📦 ${totalBarnUsed}//${totalBarnTotal}\n`;

    data.forEach((item, index) => {
        let name = cleanValue(item["账号"] || `账号${index + 1}`);
        let level = cleanValue(item["账号等级"] || "-");
        let coins = num(item["金币"]);
        let diamonds = num(item["钻石"]);
        let silo = parseCapacity(item["粮仓容量"]);
        let barn = parseCapacity(item["货仓容量"]);
        text += `\n[${index + 1}] ${name}\n`;
        text += `👤 等级 ${level}  ·  金币 ${coins}  ·  钻石 ${diamonds}\n`;
        if (silo) text += `🌾 粮仓：${silo.used}//${silo.total}\n`;
        if (barn) text += `📦 货仓：${barn.used}//${barn.total}\n`;
        text += `🧱 ${["盒钉","螺钉","镶板","螺栓","木板","胶带"].map(k => `${k} ${num(item[k])}`).join("  ·  ")}\n`;
        text += `🗺️ ${["土地契约","木槌","标桩"].map(k => `${k} ${num(item[k])}`).join("  ·  ")}\n`;
        text += `🛠️ ${["斧头","木锯","炸药","炸药桶","铁铲","十字镐"].map(k => `${k} ${num(item[k])}`).join("  ·  ")}\n`;
    });

    text += "\n总计材料\n";
    text += `🧱 ${["盒钉","螺钉","镶板","螺栓","木板","胶带"].map(k => `${k} ${itemTotals[k] || 0}`).join("  ·  ")}\n`;
    text += `🗺️ ${["土地契约","木槌","标桩"].map(k => `${k} ${itemTotals[k] || 0}`).join("  ·  ")}\n`;
    text += `🛠️ ${["斧头","木锯","炸药","炸药桶","铁铲","十字镐"].map(k => `${k} ${itemTotals[k] || 0}`).join("  ·  ")}`;
    return text;
}
