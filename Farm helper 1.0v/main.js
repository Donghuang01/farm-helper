                    ...
                    <horizontal gravity="center_vertical">
                        ...
                        <vertical padding="10 8" h="auto" w="0" layout_weight="1">
                            <text id="title" text="{{this.title}}" textColor="#333333" textSize="16sp" maxLines="1" />
                                                        <text id="friendTag" text="{{this.friendTag}}" textColor="#888888" textSize="12sp" maxLines="1" />
                        </vertical>
                        <horizontal gravity="center_vertical">
                                    ...
                                    <vertical padding="16">
                                        ...
                                        <vertical id="storageUpgradeContainer" gravity="center_vertical" visibility="gone">
                                            ...
                                            <horizontal gravity="center_vertical" marginTop="8">
                                                <text text="先互加好友" textSize="14" w="100" marginRight="8" />
                                                <Switch id="storageUpgrade_makeFriendsFirst" checked="false" />
                                                <text text="开启后会先让本轮账号互相加好友并接受请求，再开始升仓" textSize="12" textColor="#888888" marginLeft="8" />
                                            </horizontal>
                                        </vertical>
                                ...
                                <card w="*" h="auto" marginBottom="12" cardCornerRadius="8" cardElevation="2">
                                    <vertical padding="16">
                                        ...
                                        <vertical id="accountListDisplay" marginTop="8">
                                            ...
                                            <text text="邮箱账号可填：账号名|#玩家标签（标签用于加游戏好友，不是Supercell ID）" textSize="12" textColor="#888888" />
                                        </vertical>
                                        {/* 邮箱账号输入框 */}
                                        <list id="AccountList" h="auto">
                                            <card w="*" h="56" margin="0 5" cardCornerRadius="5dp"
                                                cardElevation="1dp" foreground="?selectableItemBackground">
                                                <horizontal gravity="center_vertical">
                                                    <frame h="*" w="10" bg="#f27272" />
                                                    <vertical padding="10 8" h="auto" w="0" layout_weight="1">
...
function showAnnouncement(silence = false) {
    threads.start(() => {
        try {
            let announcementUrl = "https://raw.githubusercontent.com/Donghuang01/farm-helper/main/announcement.json";
            let response = http.get(announcementUrl, { headers: { "Authorization": "token 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" } });
...
function downloadZip_dialogs() {
    ...
    dialogs.build({
        ...
        threads.start(() => {
            try {
                // 发送HTTP请求获取最新版本信息
                let apiUrl = "https://raw.githubusercontent.com/Donghuang01/farm-helper/main/versionV2.json";
                log("请求API地址: " + apiUrl);
                let response = http.get(apiUrl, {
                    headers: {
                        "Accept": "application/json",
                        "Authorization": "token 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"
                    }
                });
                        ...
                        dialogs.build({
                            ...
                            dialog.getView().connectivityBtn.on("click", () => {
                                ...
                                let giteeThread = threads.start(function () {
                                    ...
                                    try {
                                        let response = http.get("https://raw.githubusercontent.com/Donghuang01/farm-helper/main/versionV2.json", { headers: { "Authorization": "token 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" } });
...
ui.telegramCmdStartBtn.setOnClickListener(() => {
    ...
    let botToken = configs.get("telegramBotToken", "") || configs.get("token", "");
    if (!chatId || !botToken) {
        dialogs.build({
            title: "Telegram指令监听",
            content: "请先填写Telegram chat_id和token，再启动监听。",
            positive: "知道了"
        }).show();
        return;
    }
    ...
    configSaveAccountList.forEach(account => {
        if (folderNameMap[account.title]) {
            orderedSaveAccountList.push({
                ...
                shengcang_h: account.shengcang_h || { enabled: true },
                shengcang_l: account.shengcang_l || { enabled: true },
...
ui['AccountList'].on('item_click', function (item, i, itemView) {
    ...
    let oldFriendTag = item.friendTag || "";
    let editView = ui.inflate(
        <vertical padding="18 16 18 8">
            <text text="账号：" textSize="16sp" textColor="#333333" />
            <input id="titleInput" hint="账号名" text={oldTitle} />
            <text text="标签：" textSize="16sp" textColor="#333333" marginTop="12" />
            <input id="tagInput" hint="#玩家标签（例如 #ABCD123）" text={oldFriendTag} />
            <text text="标签用于加游戏好友，不是 Supercell ID；可以留空。" textSize="12sp" textColor="#888888" marginTop="10" />
        </vertical>
    );
    dialogs.build({
        title: '修改邮箱账号',
        ...
        try {
            let newTitle = String(editView.titleInput.getText() || '').trim();
            let newFriendTag = String(editView.tagInput.getText() || '').trim();
            if (!newTitle) {
                toastLog('账号名不能为空');
                return;
            }
            if (newFriendTag && !newFriendTag.startsWith('#')) {
                newFriendTag = '#' + newFriendTag;
            }
            item.title = newTitle;
            item.friendTag = newFriendTag;
            ui.run(function () {
                if (ui['AccountList'] && ui['AccountList'].adapter) {
...
function getConfig() {
    ...
    const storedConfig = {
        ...
        telegramBotToken: configs.get("telegramBotToken", configs.get("token", defaultConfig.telegramBotToken || defaultConfig.token)),
        ...
        storageUpgrade_makeFriendsFirst: configs.get("storageUpgrade_makeFriendsFirst", defaultConfig.storageUpgrade_makeFriendsFirst),
    };
    return storedConfig;
}
...
function saveConfig(con) {
    try {
        ...
        configs.put("telegramBotToken", (con.telegramBotToken !== undefined && con.telegramBotToken !== "") ? con.telegramBotToken : (con.token !== undefined ? con.token : defaultConfig.telegramBotToken));
        ...
        configs.put("storageUpgrade_makeFriendsFirst", con.storageUpgrade_makeFriendsFirst !== undefined ? con.storageUpgrade_makeFriendsFirst : defaultConfig.storageUpgrade_makeFriendsFirst);
...
function validateConfig(config) {
    ...
    // 确保邮箱账号至少保留 title / done / friendTag 字段
    config.accountList = config.accountList.map(account => {
        return {
            title: account && account.title !== undefined ? account.title : "",
            done: account && account.done !== undefined ? account.done : false,
            friendTag: account && account.friendTag ? String(account.friendTag).trim() : ""
        };
    });
...
function getDefaultConfig() {
    return {
        ...
        // 升仓互加好友
        storageUpgrade_makeFriendsFirst: false,
...
function loadConfigToUI(loadConfigFromFile = false) {
    ...
    ui.storageUpgrade_makeFriendsFirst.setChecked(!!config.storageUpgrade_makeFriendsFirst);
    ...
    dialogs.build({
        ...
        customView: ui.inflate(
            <vertical>
                ...
                <text text="本软脚本免费开源" />
                <text text="" />
                <text autoLink="web" text="开源地址: https://github.com/Donghuang01/farm-helper" />
                <text text="" />
...
function initUI() {
    ...
    ui.storageUpgrade_makeFriendsFirst.on("check", function(checked) {
        configs.put("storageUpgrade_makeFriendsFirst", checked);
    });
