            ...
            if (response.statusCode == 200) {
                ...
                ui.run(() => {
                    if (compareResult < 0) {
                        ...
                        // 当前版本更新（开发中）
                        toastLog("你的版本超过了全球100%的用户！你正在使用开发版" + projectData.versionName + " > " + latestVersion, "long");
                    } else if (!silence && compareResult == 0) {
                        // 没有新版本
...
function showDonateDialog() {
    dialogs.build({
        title: "🌟支持我们🌟 ",
        content: "如果觉得 farm-helper 好用，欢迎给个 Star ⭐\n" +
            "您的支持是我更新的动力！",
        positive: "去点 Star",
        neutral: "下次一定",
    }).on("positive", () => {
        app.openUrl("https://github.com/Donghuang01/farm-helper");
    }).on("neutral", () => {
        toast("没关系，欢迎下次再来 😊");
    }).show();
}
